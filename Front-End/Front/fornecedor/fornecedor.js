...
async function buscaDaApiExibeNaTabela() {
    const res = await fetch(`${URL_BASE_API}/fornecedor/listar`);
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        td2.innerText = data[i].endereço;
        td3.innerText = data[i].telefone;
...
async function preencheDadosParaEdicao(e) {
    ...
    const res = await fetch(`${URL_BASE_API}/fornecedor/um`, opt);
    const data = await res.json();
    console.log(data);
    document.forms[0].id.value = data.id;
    document.forms[0].nome.value = data.nome;
    document.forms[0].endereço.value = data.endereço;
    document.forms[0].telefone.value = data.telefone;
}
...
async function enviaDadosParaCadastroOuEdicaoNaApi(e) {
    ...
    const res = await fetch(`${URL_BASE_API}/fornecedor/${op}`, opt);
    const data = await res.json();
    e.target.reset();
...
async function solicitaExclusaoDeCartaNaApi(e) {
    ...
    const res = await fetch(`${URL_BASE_API}/fornecedor/excluir`, opt);
    const data = await res.json();
    alert(data.status);
    buscaDaApiExibeNaTabela();
}
